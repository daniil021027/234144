package ru.ac.uniyar.rofjlaHbl;
    ...
    public boolean deleteNode(int value) // Удаление узла с заданным ключом
    {
        ...
        if (currentNode.getLeftChild() == null && currentNode.getRightChild() == null) { // узел просто удаляется, если не имеет потомков
            if (currentNode == rootNode) // если узел - корень, то дерево очищается
                rootNode = null;
            else if (isLeftChild) parentNode.setLeftChild(null); // если нет - узел отсоединяется, от родителя
...
public class VVOD {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            tree.insertNode(znach);
            //int znach1=in.nextInt();
            //  tree.insertNode(znach1);

        }
        tree.printTree();
        //tree.deleteNode(znach1);
        // int znach2= in.nextInt();
        //  Node foundNode=tree.findNodeByValue(znach2);
        // foundNode.printNode();
    }
}